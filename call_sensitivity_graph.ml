...
module Calltable = struct
  ...
  let encode tab = Sexp.to_string (String.Table.sexp_of_t sexp_of_kcs tab)
  let decode str = String.Table.t_of_sexp kcs_of_sexp @@ Sexp.of_string str
end

module DotDefaults = struct
...
module LCSG = struct
  ...
  let of_table (filename : string) : t =
    let table = Calltable.decode (In_channel.read_all filename) in
    Calltable.fold table ~init:empty
